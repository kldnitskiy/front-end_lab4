  ...
  constructor(private http: HttpClient, private router: Router) {
    ...
    login(){
      ...
      if (this.loginVerify && this.passwordVerify){
        this.response = '';
        // @ts-ignore
        this.http.post('rest/users/login', 'username=' + encodeURIComponent(this.userName) + '&pwd=' + this.password, {headers, responseType: 'text', observe: 'response'}).pipe(catchError(err => {
          console.log(err);
